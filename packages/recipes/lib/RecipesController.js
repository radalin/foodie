RecipesController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('recipes', this.getCriteria(), sort);
    },
...
{up_votes_count: {$regex: search, $options: 'i'}},
...
{updated_at: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Recipes.find(this.getCriteria(), sort);

        return this.render('recipesIndex', {
            data: {
    ...
    view: function() {
        return this.render('recipesView', {
            data: {
...
preparation: t.find('#preparation').value,
up_votes_count: t.find('#up_votes_count').value? Number(t.find('#up_votes_count').value):null,
created_at: t.find('#created_at').value? new Date(t.find('#created_at').value):null,
updated_at: t.find('#updated_at').value? new Date(t.find('#updated_at').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Recipes.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', __("insert_success", "") + ".");
                Router.go('recipesView', {_id: _id});
            });
        }
        return this.render('recipesInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Recipes.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
        ...
        return this.render('recipesUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Recipes.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
